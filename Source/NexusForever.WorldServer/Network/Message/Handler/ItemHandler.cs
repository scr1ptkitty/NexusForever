...
namespace NexusForever.WorldServer.Network.Message.Handler
{
    public static class ItemHandler
    {
        ...
        public static void HandleItemUseDecor(WorldSession session, ClientItemUseDecor itemUseDecor)
        {
            ...
            Task<Game.Housing.Residence> task = ResidenceManager.Instance.GetResidence(session.Player.Name);
            session.EnqueueEvent(new TaskGenericEvent<Game.Housing.Residence>(task,
                residence =>
            {
                if (residence == null)
